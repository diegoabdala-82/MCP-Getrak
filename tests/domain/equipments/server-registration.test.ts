...
import { registerEquipmentTools } from "../../../src/domain/equipments/index.js";
import { createGetrakMcpServer } from "../../../src/server.js";
import { createFakeApiCoreClient } from "./test-helpers.js";

describe("Epic 4 — registro das 2 tools de equipamentos no servidor MCP", () => {
  it("expõe as 2 tools na descoberta nativa", async () => {
    ...
    registerEquipmentTools(registerDomainTool, { apiCoreClient: createFakeApiCoreClient([]).client });
    ...
    expect(names).toEqual(["get_equipment_bench_position", "search_equipments"]);
  });
});
