...
describe("get_fuel_supply_details — integração via ToolRuntime", () => {
  ...
  it("normaliza abastecimento inexistente para FUEL_SUPPLY_NOT_FOUND", async () => {
    ...
    expect(auditSink.records[0]).toMatchObject({ tool: "get_fuel_supply_details", result: "error", error_code: "FUEL_SUPPLY_NOT_FOUND" });
  });
});
