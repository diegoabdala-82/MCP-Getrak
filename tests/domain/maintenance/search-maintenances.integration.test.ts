...
describe("search_maintenances — integração via ToolRuntime", () => {
  ...
  it("aplica auditoria/envelope de sucesso via ToolRuntime", async () => {
    const fake = createFakeApiCoreClient({ data: [{ id: 1 }], page: 1, pages: 1, total: 1 });
    const delegated = createFakeDelegatedTokenManager();
    const { definition } = createSearchMaintenancesTool({ apiCoreClient: fake.client, delegatedTokenManager: delegated.manager });
    ...
    expect(envelope).toMatchObject({ data: { maintenances: [{ id: 1 }] } });
    expect(auditSink.records[0]).toMatchObject({ tool: "search_maintenances", result: "success" });
  });
});
