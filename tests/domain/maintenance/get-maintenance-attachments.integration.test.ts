...
describe("get_maintenance_attachments — integração via ToolRuntime", () => {
  ...
  it("normaliza manutenção inexistente para MAINTENANCE_NOT_FOUND", async () => {
    ...
    expect(auditSink.records[0]).toMatchObject({ tool: "get_maintenance_attachments", result: "error", error_code: "MAINTENANCE_NOT_FOUND" });
  });
});
