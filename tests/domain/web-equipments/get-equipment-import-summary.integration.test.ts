...
describe("get_equipment_import_summary — integração via ToolRuntime", () => {
  ...
  it("normaliza requisição de importação inexistente para EQUIPMENT_IMPORT_NOT_FOUND", async () => {
    const fake = createRejectingApiCoreClient(
      new McpToolError({ code: "EQUIPMENT_IMPORT_NOT_FOUND", message: "Import not found.", retryable: false }),
    );
    const delegated = createFakeDelegatedTokenManager();
    const { definition } = createGetEquipmentImportSummaryTool({ apiCoreClient: fake.client, delegatedTokenManager: delegated.manager });
    ...
    expect(auditSink.records[0]).toMatchObject({ tool: "get_equipment_import_summary", result: "error", error_code: "EQUIPMENT_IMPORT_NOT_FOUND" });
  });
});
