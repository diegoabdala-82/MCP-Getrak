...
describe("get_web_equipment_details — integração via ToolRuntime", () => {
  it("bloqueia cross-central antes de chamar a API Core", async () => {
    ...
    const envelope = await runtime.execute(
      definition,
      { central: "central-99", serial_number: "B123" },
  ...
  it("normaliza equipamento inexistente para EQUIPMENT_NOT_FOUND", async () => {
    const fake = createRejectingApiCoreClient(
      new McpToolError({ code: "EQUIPMENT_NOT_FOUND", message: "Equipment not found.", retryable: false }),
    );
    const delegated = createFakeDelegatedTokenManager();
    const { definition } = createGetWebEquipmentDetailsTool({ apiCoreClient: fake.client, delegatedTokenManager: delegated.manager });
    ...
    const envelope = await runtime.execute(
      definition,
      { central: "central-1", serial_number: "NOPE" },
      { consumer: { consumer_id: "consumer-a" }, environment: "homologation" },
    );

    expect(envelope).toMatchObject({ error: { code: "EQUIPMENT_NOT_FOUND" } });
    expect(auditSink.records[0]).toMatchObject({ tool: "get_web_equipment_details", result: "error", error_code: "EQUIPMENT_NOT_FOUND" });
  });
});
