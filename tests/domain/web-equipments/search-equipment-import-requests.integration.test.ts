...
describe("search_equipment_import_requests — integração via ToolRuntime", () => {
  ...
  it("aplica auditoria/envelope de sucesso via ToolRuntime", async () => {
    const fake = createFakeApiCoreClient({ data: [{ id: 126 }], page: 1, pages: 1, total: 1 });
    const delegated = createFakeDelegatedTokenManager();
    const { definition } = createSearchEquipmentImportRequestsTool({ apiCoreClient: fake.client, delegatedTokenManager: delegated.manager });
    ...
    expect(envelope).toMatchObject({ data: { import_requests: [{ id: 126 }] } });
    expect(auditSink.records[0]).toMatchObject({ tool: "search_equipment_import_requests", result: "success" });
  });
});
