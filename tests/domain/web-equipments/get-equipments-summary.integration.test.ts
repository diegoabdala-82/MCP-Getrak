...
describe("get_equipments_summary — integração via ToolRuntime", () => {
  ...
  it("aplica auditoria/envelope de sucesso via ToolRuntime", async () => {
    const fake = createFakeApiCoreClient({ active: 1, total: 1 });
    const delegated = createFakeDelegatedTokenManager();
    const { definition } = createGetEquipmentsSummaryTool({ apiCoreClient: fake.client, delegatedTokenManager: delegated.manager });
    ...
    expect(auditSink.records[0]).toMatchObject({ tool: "get_equipments_summary", result: "success" });
  });
});
