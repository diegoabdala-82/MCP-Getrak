...
describe("search_equipment_carriers — integração via ToolRuntime", () => {
  ...
  it("aplica auditoria/envelope de sucesso via ToolRuntime", async () => {
    const fake = createFakeApiCoreClient({ data: [{ name: "OI" }], page: 1, pages: 1, total: 1 });
    const delegated = createFakeDelegatedTokenManager();
    const { definition } = createSearchEquipmentCarriersTool({ apiCoreClient: fake.client, delegatedTokenManager: delegated.manager });
    ...
    expect(envelope).toMatchObject({ data: { carriers: [{ name: "OI" }] } });
    expect(auditSink.records[0]).toMatchObject({ tool: "search_equipment_carriers", result: "success" });
  });
});
