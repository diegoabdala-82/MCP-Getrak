...
describe("search_equipment_tags — integração via ToolRuntime", () => {
  ...
  it("aplica auditoria/envelope de sucesso via ToolRuntime", async () => {
    const fake = createFakeApiCoreClient({ data: [{ id: 27, value: "Jennifer" }] });
    const delegated = createFakeDelegatedTokenManager();
    const { definition } = createSearchEquipmentTagsTool({ apiCoreClient: fake.client, delegatedTokenManager: delegated.manager });
    ...
    expect(envelope).toMatchObject({ data: { tags: [{ id: 27, value: "Jennifer" }] } });
    expect(auditSink.records[0]).toMatchObject({ tool: "search_equipment_tags", result: "success" });
  });
});
