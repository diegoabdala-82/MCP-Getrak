...
describe("search_inventory — integração via ToolRuntime", () => {
  ...
  it("aplica auditoria/envelope de sucesso via ToolRuntime", async () => {
    const fake = createFakeApiCoreClient({ data: [{ model: { id: 3 } }], page: 1, pages: 1, total: 1 });
    const delegated = createFakeDelegatedTokenManager();
    const { definition } = createSearchInventoryTool({ apiCoreClient: fake.client, delegatedTokenManager: delegated.manager });
    ...
    expect(envelope).toMatchObject({ data: { inventory: [{ model: { id: 3 } }] } });
    expect(auditSink.records[0]).toMatchObject({ tool: "search_inventory", result: "success" });
  });
});
