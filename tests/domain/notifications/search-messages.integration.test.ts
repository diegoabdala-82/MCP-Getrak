...
describe("search_messages — integração completa via ToolRuntime", () => {
  it("aplica autenticação delegada/validação de central/envelope/auditoria sem duplicar lógica na tool", async () => {
    const fake = createFakeApiCoreClient({ data: [{ id: 1, title: "Aviso" }], page: 1, pages: 1, total: 1 });
    ...
    expect(envelope).toMatchObject({ data: { messages: [{ id: 1, title: "Aviso" }] } });
  ...
  it("normaliza falha de endpoint indisponível/timeout da API Core no envelope de erro padrão", async () => {
    ...
    expect(auditSink.records[0]).toMatchObject({ tool: "search_messages", result: "error", error_code: "UPSTREAM_UNAVAILABLE" });
  });
});
