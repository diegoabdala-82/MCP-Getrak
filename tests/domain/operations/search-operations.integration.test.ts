...
describe("search_operations — integração completa via ToolRuntime", () => {
  it("aplica autenticação delegada/validação de central/envelope/auditoria sem duplicar lógica na tool", async () => {
    const fake = createFakeApiCoreClient({ data: [{ id: 1, type: "device" }], page: 1, pages: 1, total: 1 });
    ...
    const envelope = await runtime.execute(
      definition,
      { central: "central-1", operation_types: ["device"], entity_id: "12927131", date: "2026-08-17" },
      { consumer: { consumer_id: "consumer-a" }, environment: "homologation" },
    );

    expect(envelope).toMatchObject({ data: { operations: [{ id: 1, type: "device" }] } });
  ...
  it("retorna VALIDATION_ERROR e não chama a API Core quando falta um filtro obrigatório", async () => {
    ...
    const envelope = await runtime.execute(
      definition,
      { central: "central-1", operation_types: ["device"], entity_id: "12927131" },
      { consumer: { consumer_id: "consumer-a" }, environment: "homologation" },
    );
    ...
    expect(auditSink.records[0]).toMatchObject({ tool: "search_operations", result: "error", error_code: "VALIDATION_ERROR" });
  });
  ...
  it("bloqueia cross-central antes de chamar a API Core", async () => {
    ...
    const envelope = await runtime.execute(
      definition,
      { central: "central-99", operation_types: ["device"], entity_id: "12927131", date: "2026-08-17" },
      { consumer: { consumer_id: "consumer-a" }, environment: "homologation" },
    );
  ...
  it("normaliza falha de endpoint indisponível/timeout da API Core no envelope de erro padrão", async () => {
    ...
    expect(auditSink.records[0]).toMatchObject({ tool: "search_operations", result: "error", error_code: "UPSTREAM_UNAVAILABLE" });
  });
});
