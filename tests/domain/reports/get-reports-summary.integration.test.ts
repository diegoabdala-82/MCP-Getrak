...
describe("get_reports_summary — integração completa via ToolRuntime", () => {
  it("aplica autenticação delegada/validação de central/envelope/auditoria sem duplicar lógica na tool", async () => {
    ...
    expect(envelope).toMatchObject({ data: { summary: { schedules: 51 } } });
    expect(auditSink.records[0]).toMatchObject({
      tool: "get_reports_summary",
      result: "success",
  ...
  it("normaliza falha de endpoint indisponível/timeout da API Core no envelope de erro padrão", async () => {
    ...
    expect(auditSink.records[0]).toMatchObject({ tool: "get_reports_summary", result: "error", error_code: "UPSTREAM_UNAVAILABLE" });
  });
});
