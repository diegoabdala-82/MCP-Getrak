...
describe("get_entity_import_details — integração via ToolRuntime", () => {
  ...
  it("normaliza requisição de importação inexistente (HTTP 404 real) para ENTITY_IMPORT_NOT_FOUND", async () => {
    const fake = createRejectingApiCoreClient(
      new McpToolError({ code: "ENTITY_IMPORT_NOT_FOUND", message: "import not found.", retryable: false }),
    );
    const delegated = createFakeDelegatedTokenManager();
    const { definition } = createGetEntityImportDetailsTool({ apiCoreClient: fake.client, delegatedTokenManager: delegated.manager });
    ...
    expect(auditSink.records[0]).toMatchObject({ tool: "get_entity_import_details", result: "error", error_code: "ENTITY_IMPORT_NOT_FOUND" });
  });
});
