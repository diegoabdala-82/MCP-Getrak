...
describe("search_web_vehicles — integração completa via ToolRuntime", () => {
  it("aplica autenticação delegada/validação de central/envelope/auditoria sem duplicar lógica na tool", async () => {
    ...
    expect(envelope).toMatchObject({ data: { vehicles: [{ id: 1, plate: "NYC1D62" }] } });
    expect(auditSink.records[0]).toMatchObject({
      tool: "search_web_vehicles",
      result: "success",
